...
class CreateSpeedVerifyLogTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('speed_verify_log', function (Blueprint $table) {
            ...
            $table->tinyInteger('is_sent', 1)->default(0)->comment('是否发送成功，0等待发送,1发送成功,-1发送失败');
            $table->tinyInteger('is_verify', 1)->default(0)->comment('是否验证，0未验证,1已验证');
            $table->text('result')->nullable()->comment('请求接口返回数据(过长信息将被截断)');
            $table->string('client_ip', 128)->nullable()->comment('客户端请求IP');
            $table->integer('expire_time', 10)->default(0)->comment('过期时间');
            $table->integer('update_time', 10)->default(0)->comment('更新时间');
            $table->integer('create_time', 10)->default(0)->comment('创建时间');

            $table->unique('verify_mode');
            $table->unique('verify_to');
            $table->unique('verify_template');
            $table->unique('verify_code');
            $table->unique('is_verify');
        });
    }
    /**
